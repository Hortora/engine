...
**`GardenMcpTools`** — `@Tool`-annotated CDI bean. `garden_search` calls `SearchResource.searchAdaptive()` and formats full entry text for LLM consumption with provenance labels (`[own]` for local, `[prefix]` for remote), CE score display, and trimmed-result headers. `garden_status` returns index count (via `EmbeddingIngestor.listDocuments()`) and garden path.
