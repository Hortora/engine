...
- **BGE-M3 via MultiModalEmbedder** — `HybridSearchProducer` produces a single `MultiModalEmbedder` bean from `@Inference("bge-m3")` via `@LookupIfProperty`; non-resolvable when the ONNX model path isn't configured. `MultiModalEmbedder` replaces the previous `EmbeddingModel` + `SparseEmbedder` + `CrossEncoderReranker` triple. Optional Matryoshka truncation via `casehub.rag.matryoshka.dimension` — wraps with `MatryoshkaMultiModalEmbedder` (truncate + L2 re-normalize). `CollectionMigration` uses `CollectionCompatibility.check()` → sealed `MigrationAction` switch to detect dimension mismatch, missing sparse vectors, or missing ColBERT config and trigger re-indexing. Also validates ColBERT multi-vector size at startup (`maxSequenceLength × colbertDimension ≤ maxMultivectorFloats`) — fails fast before any data reaches Qdrant. Clears stale cursors when the Qdrant collection is absent — prevents silent zero-ingestion on fresh deployments.
...
- **CBR outcome tracking** — `GardenOutcomeService` stores `ResolvedCase` per GE-ID via `CbrCaseMemoryStore` SPI (H2 file-persistent at `~/.hortora/stats/cbr`). Store-once/record-many lifecycle: first outcome creates the case via `store()`, subsequent outcomes adjust confidence via `recordOutcome()`. `gardenRecordOutcome` MCP tool + REST `POST /api/garden/outcomes` for recording. `gardenOutcomeReport` MCP tool + REST `GET /api/garden/outcomes/report` surfaces entries sorted by trust score via `scan()`. Operates at per-issue granularity (not per-retrieval — complements, not replaces, `RetrievalTracker.feedback()`).
- **HyDE query expansion (definitively closed)** — four approaches benchmarked (double-retrieval, single-retrieval, inverted HyDE, no-HyDE baseline), all -2.2 to -2.5pp precision. Root cause confirmed by Weller et al. (EACL 2024): expansion harms strong multi-signal retrievers. All HyDE infrastructure removed (`OllamaQueryGenerator`, `QueryAugmentingExtractor`, `QueryGenerator`, `InvertedHydeConfig` and config stanza deleted). Benchmark data preserved in `docs/comparison/`.
...
- **Post-retrieval scoring modifiers** — `TemporalDecayScorer` and `VersionScorer` both implement `PostRetrievalScorer` SPI (`adjust(RetrievedChunk, RetrievalQuery, ScoringContext)`). Applied via `SearchResource.applyScoring()` as configurable multipliers after retrieval: temporal half-life decay by staleness tier (fast/standard/slow/evergreen) and BOM-relative version distance with topic weighting. Both enabled by default via `SearchScoringConfig` (prefix `hortora.search.scoring`). Multipliers compose: temporal × version × base score.
- **Search profiles for BOM-based version scoring** — `SearchProfileStore` (SQLite) + `ProfileResource` (REST CRUD at `/api/garden/profiles/{name}`) stores BOM snapshots (name→version maps). `gardenSearch` MCP tool accepts `profile` (named BOM) or `stack` (inline pipe-separated pairs like `quarkus:3.36.1|jdk:26.0.2`). Inline `stack` takes precedence over named profile.
- **Frontmatter field promotion to REST API** — `GardenMetadataExtractor` writes `staleness_threshold`, `tags`, `last_reviewed`, `author`, `verified_on` as Qdrant payload fields at ingestion. `SearchResult` exposes them as top-level JSON fields via derived `@JsonProperty` methods (null-guarded for federation-deserialized instances where the internal metadata map is null). Backfill happens naturally on reindex.
