...
- **Post-retrieval scoring modifiers** — `SearchResource.applyScoring()` applies two configurable multipliers after retrieval: `TemporalDecayScorer` (half-life decay by staleness tier — fast/standard/slow/evergreen, derived from `staleness_threshold` frontmatter) and `VersionScorer` (BOM-relative version distance with topic weighting — entries about outdated stack versions get proportional score reduction). Both enabled by default via `SearchScoringConfig` (prefix `hortora.search.scoring`). Multipliers compose: temporal × version × base score.
